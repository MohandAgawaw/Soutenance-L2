...
firebase.auth().onAuthStateChanged(function(user) {
  if(user){
      ...
      if (user != null) {
        ...
        firebase.database().ref('users/' + user.uid+'/CBCHAT').on('value',(snapp)=>{
          chattyp = snapp.val()
          
          if(chattyp =='Banned'){

            alert('Vous etes Banni Du chat...')
...
function star() {
  ...
  firebase.database().ref("messages").on("child_added", function (snapshot) {
    if (snapshot.val().sender == myName) {
      ...
      $('<div class="message new"><figure class="avatar"><img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQpdX6tPX96Zk00S47LcCYAdoFK8INeCElPeJrVDrh8phAGqUZP_g" /></figure><div id="message-' + snapshot.key + '">' + snapshot.val().name + ': ' + snapshot.val().message + '</div></div>').appendTo($('.mCSB_container')).addClass('new');
    }
